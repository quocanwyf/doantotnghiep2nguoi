...
Nếu chọn phương án thứ nhất, đề xuất **MobileFaceNet** làm model nhận dạng cần khảo sát. Giả thuyết nghiên cứu là đặc trưng của model nhẹ có thể kém ổn định khi ảnh check-in mờ, thiếu sáng hoặc khác đáng kể ảnh đăng ký. Thuật toán optimization **có thể** tìm chính sách tăng cường dữ liệu cho giai đoạn fine-tune; chỉ giữ hướng này nếu thử nghiệm ban đầu xác nhận lỗi thực sự nằm ở nhận dạng. Dataset đề xuất gồm **DigiFace-1M cho huấn luyện nghiên cứu**, **ảnh người tham gia đồng ý cung cấp để đăng ký và đánh giá đúng bối cảnh**, và **LFW để kiểm tra tham khảo theo giao thức xác minh cặp ảnh**. Quyền sử dụng và chi phí huấn luyện cần được xác nhận trước khi lấy dữ liệu.
...
**Model nghiên cứu:** MobileFaceNet tạo embedding khuôn mặt. Model phát hiện mặt được giữ cố định ở cả baseline và proposed, ví dụ SCRFD-500MF nếu giấy phép và thử nghiệm trên điện thoại phù hợp. Việc dùng detector không biến đề tài thành tối ưu hai model; đối tượng thay đổi trong phép so sánh chính là **cách fine-tune MobileFaceNet**. [Bài báo MobileFaceNets](https://arxiv.org/abs/1804.07573) thiết kế kiến trúc cho thiết bị di động. [ArcFace](https://openaccess.thecvf.com/content_CVPR_2019/html/Deng_ArcFace_Additive_Angular_Margin_Loss_for_Deep_Face_Recognition_CVPR_2019_paper.html) là một loss có thể dùng khi huấn luyện, không phải bộ nhận dạng thứ hai lúc chạy app.
...
**Giả thuyết điểm yếu:** trên camera dự kiến, ảnh mờ do chuyển động, giảm độ phân giải hoặc thay đổi sáng làm embedding của cùng người kém ổn định, tăng FNIR hoặc nhận sai. Nghiên cứu [AdaFace](https://openaccess.thecvf.com/content/CVPR2022/papers/Kim_AdaFace_Quality_Adaptive_Margin_for_Face_Recognition_CVPR_2022_paper.pdf) chỉ ra nhận dạng mặt chất lượng thấp là một bài toán riêng; cần xác nhận bằng ảnh thực tế của nhóm. Đầu tiên hãy đo tỷ lệ lỗi ở bước phát hiện, căn chỉnh và nhận dạng riêng. Nếu phần lớn lỗi do detector hoặc ảnh quá thiếu thông tin, không gán nhầm vấn đề đó cho MobileFaceNet.

**Phương án optimization dự kiến, chưa chốt thuật toán:** tìm chính sách tăng cường dữ liệu khi fine-tune MobileFaceNet, gồm xác suất và mức độ làm mờ, giảm độ phân giải và thay đổi sáng trong phạm vi giống camera thật. Một thuật toán tìm kiếm như Bayesian optimization chọn tham số trên validation; [AutoAugment](https://research.google/pubs/autoaugment-learning-augmentation-policies-from-data/) là cơ sở cho cách tìm chính sách dựa trên chất lượng validation. Không áp dụng biến đổi làm mất danh tính hoặc tạo ảnh phi thực tế chỉ để tăng điểm số.

- **Baseline:** cùng kiến trúc MobileFaceNet, cùng khởi tạo, dữ liệu và ngân sách huấn luyện; chính sách augmentation cố định, công bố trước.
- **Proposed:** thay đúng chính sách augmentation bằng chính sách được thuật toán tìm; fine-tune lại MobileFaceNet. **Đăng ký embedding không được tính là train/fine-tune.**
- **Objective:** giảm FNIR trên dữ liệu phát triển tại mức FPIR đã định; test cuối được giữ riêng. Vì kiến trúc inference giữ nguyên, không hứa tăng tốc hoặc giảm kích thước; đo lại để xác nhận. So với random search cùng số lần đánh giá để biết thuật toán tìm kiếm có lợi ích riêng hay không.
- **Điều kiện dừng hướng này:** thử nghiệm ban đầu không cho thấy lỗi nhận dạng dưới thay đổi chất lượng, dữ liệu không đủ/không được phép dùng, hoặc chi phí fine-tune nhiều lần vượt khả năng nhóm. Khi đó nhóm phải chọn lại điểm tối ưu theo bằng chứng, không tuyên bố đã cải thiện.
...
3. Yêu cầu train/fine-tune và thuật toán optimization: tìm chính sách augmentation ở training time có được chấp nhận là đóng góp chính không?
