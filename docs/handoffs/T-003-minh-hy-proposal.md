...
- **Commit/PR:** Báo cáo ban đầu ở commit `8a1a679`, phần bổ sung optimization ở `ee6de56`, phần sắp xếp theo lộ trình nghiên cứu ở `b598afc`, trên nhánh `task/T-003-minh-hy-proposal`. PR chưa tạo do GitHub từ chối push với mã 403 cho tài khoản `Minh-Hy`.
- **Tóm tắt và file chính:** [Bản đề xuất](../01-problem/minh-hy-proposal.md) so sánh check-in từng người với chụp nhiều người trong lớp; trình bày người dùng, camera, đầu ra, logic điểm danh, tiêu chí đo và giả định. Lộ trình đi từ bài toán → yêu cầu → dữ liệu/dataset → model → điểm tối ưu → thuật toán → pipeline/thực nghiệm. MobileFaceNet và các nguồn dữ liệu vẫn là ứng viên; hai hướng optimization là tham số huấn luyện hoặc bounding box/ROI. Báo cáo định nghĩa chỉ số và cách đo mức thay đổi so với baseline, chưa có số liệu của nhóm.
